...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        var chart_base_x=1060;//1050
        var chart_base_x_axis = 1060;
        var chart_base_xl=-120;
        var chart_base_y=accordion_bar_y + 25;//80
        ...
        accordion_unit.append("rect")
                      ...
                      .attr("visibility",function(d, i){if(i>2&&i<10) return "hidden";})
...
$('.menu').addClass('fixed');
  $(window).bind('scroll', function () {
      ...
      if (windowWidth/windowHeight < 1480/800){
        scrollScale = 1480/windowWidth;
      }
      else{
        scrollScale = 800/windowHeight;
      }
      d3.select("#centralChart").attr("transform","translate(0,"+windowScroll*scrollScale +")");
  });
  $( function() {
    $( "#dialog" ).dialog({
      autoOpen: false,
...
<script>
  ...
  // initial parameters
  var oldmin=-1;oldmax=-1;oldmaini=-1;
  var oldmin2=-1;oldmax2=-1;oldmaini2=-1;
  var college = {
    name : [],
    number : [],
    style : [],
...
var occupationhispanicPercent = {
  ...
  var college_name= { name : []};
  var major_character_requirement = [];
  var major_character_meanrequirement = [];
  var major_salary = [];
  var major_meansalary = [];
  var major_meansalaryrange = [];
  var major_SAT = [];
  var major_meanSAT = [];
  var major_meanSATrange = [];
  var major_GPA = [];
  var major_meanGPA = [];
  var major_meanGPArange = [];
  var occupation_character_requirement = [];
  var occupation_character_meanrequirement = [];
  var occupation_salary = [];
  var occupation_meansalary = [];
  var occupation_meansalaryrange = [];
...
while($rows = mysqli_fetch_assoc($result))
  ...
  description from MajorCha042016  group by major, college";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
...
<!--  extract major salary -->
  <?php
  $sql ="SELECT major, college, observations, verbalSkill, quantitativeSkill, reasoningSkill, percentFemale, 
  salary10thPercentile, salary25thPercentile, salaryMedian, salary75thPercentile, salary90thPercentile from MajorCha042016  group by college,major";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
   while($rows = mysqli_fetch_assoc($result))
  {
    if($rows["salary90thPercentile"]<$rows["salary75thPercentile"])
    {
      $rows["salary90thPercentile"]=$rows["salary75thPercentile"];
    }
    if($rows["salary25thPercentile"]<$rows["salary10thPercentile"])
    {
      $rows["salary25thPercentile"]=$rows["salary10thPercentile"];
    }
    ?>
  <script>
    major_salary.push({
...
<!--  extract major SAT -->
  <?php
  $sql ="SELECT major, college, observations, verbalSkill, quantitativeSkill, reasoningSkill, percentFemale, 
  SAT10thPercentile,SAT25thPercentile, SATMedian, SAT75thPercentile, SAT90thPercentile from MajorCha042016  group by college,major";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
   while($rows = mysqli_fetch_assoc($result))
  {
    if($rows["SAT90thPercentile"]<$rows["SAT75thPercentile"])
    {
      $rows["SAT90thPercentile"]=$rows["SAT75thPercentile"];
    }
    if($rows["SAT25thPercentile"]<$rows["SAT10thPercentile"])
    {
      $rows["SAT25thPercentile"]=$rows["SAT10thPercentile"];
    }
    ?>
  <script>
    major_SAT.push({
    SAT10: parseFloat("<?php echo($rows["SAT10thPercentile"]); ?>"),
    SAT25: parseFloat("<?php echo($rows["SAT25thPercentile"]); ?>"),
    SAT50: parseFloat("<?php echo($rows["SATMedian"]); ?>"),
    SAT75: parseFloat("<?php echo($rows["SAT75thPercentile"]); ?>"),
    SAT90: parseFloat("<?php echo($rows["SAT90thPercentile"]); ?>")
  });
...
<!--  extract major GPA -->
  <?php
  $sql ="SELECT major, college, observations, verbalSkill, quantitativeSkill, reasoningSkill, percentFemale, 
  purdueGPA10thPercentile, purdueGPA25thPercentile, purdueGPAMedian, purdueGPA75thPercentile, purdueGPA90thPercentile from MajorCha042016  group by college,major";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
   while($rows = mysqli_fetch_assoc($result))
  {
    if($rows["purdueGPA90thPercentile"]<$rows["purdueGPA75thPercentile"])
    {
      $rows["purdueGPA90thPercentile"]=$rows["purdueGPA75thPercentile"];
    }
    if($rows["purdueGPA25thPercentile"]<$rows["purdueGPA10thPercentile"])
    {
      $rows["purdueGPA25thPercentile"]=$rows["purdueGPA10thPercentile"];
    }
    ?>
  <script>
    major_GPA.push({
...
<!--  extract major character mean requirement -->
  ...
  description from MajorCha042016  group by college";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
...
<!--  extract major character mean salary -->
  <?php
  $sql ="SELECT major, college, observations, verbalSkill, quantitativeSkill, reasoningSkill, percentFemale,
  sum(salary10thPercentile*observations)/sum(observations) as salary10,
  sum(salary25thPercentile*observations)/sum(observations) as salary25,
  sum(salaryMedian*observations)/sum(observations) as salary50,
  sum(salary75thPercentile*observations)/sum(observations) as salary75,
  sum(salary90thPercentile*observations)/sum(observations) as salary90 from MajorCha042016 where salary90thPercentile-salary75thPercentile>0 and salary25thPercentile-salary10thPercentile>0  group by college";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
   while($rows = mysqli_fetch_assoc($result))
  {
    ?>
  <script>
  major_meansalaryrange.push(<?php echo($rows["salary90"]); ?>);
  major_meansalaryrange.push(<?php echo($rows["salary10"]); ?>);
    major_meansalary.push({
    salary10: parseInt("<?php echo($rows["salary10"]); ?>"),
    salary25: parseInt("<?php echo($rows["salary25"]); ?>"),
    salary50: parseInt("<?php echo($rows["salary50"]); ?>"),
    salary75: parseInt("<?php echo($rows["salary75"]); ?>"),
...
<!--  extract major character mean SAT -->
  <?php
  $sql ="SELECT major, college, observations, verbalSkill, quantitativeSkill, reasoningSkill, percentFemale,
  sum(SAT10thPercentile*observations)/sum(observations) as SAT10,
  sum(SAT25thPercentile*observations)/sum(observations) as SAT25,
  sum(SATMedian*observations)/sum(observations) as SAT50,
  sum(SAT75thPercentile*observations)/sum(observations) as SAT75,
  sum(SAT90thPercentile*observations)/sum(observations) as SAT90 from MajorCha042016 group by college";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
   while($rows = mysqli_fetch_assoc($result))
  {
    ?>
  <script>
  college_name.name.push("<?php echo($rows["college"]); ?>");
  
  if(<?php echo($rows["SAT90"]); ?>>0)
  {major_meanSATrange.push(<?php echo($rows["SAT90"]); ?>);}
  if(<?php echo($rows["SAT10"]); ?>>0)
  {major_meanSATrange.push(<?php echo($rows["SAT10"]); ?>);}
  
    major_meanSAT.push({
    SAT10: parseFloat("<?php echo($rows["SAT10"]); ?>"),
    SAT25: parseFloat("<?php echo($rows["SAT25"]); ?>"),
    SAT50: parseFloat("<?php echo($rows["SAT50"]); ?>"),
    SAT75: parseFloat("<?php echo($rows["SAT75"]); ?>"),
    SAT90: parseFloat("<?php echo($rows["SAT90"]); ?>")
  });
...
<!--  extract major character mean GPA -->
  <?php
  $sql ="SELECT major, college, observations, verbalSkill, quantitativeSkill, reasoningSkill, percentFemale,
  sum(purdueGPA10thPercentile*observations)/sum(observations) as GPA10,
  sum(purdueGPA25thPercentile*observations)/sum(observations) as GPA25,
  sum(purdueGPAMedian*observations)/sum(observations) as GPA50,
  sum(purdueGPA75thPercentile*observations)/sum(observations) as GPA75,
  sum(purdueGPA90thPercentile*observations)/sum(observations) as GPA90 from MajorCha042016 group by college";
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
  ...
  major_meanGPArange.push(<?php echo($rows["GPA90"]); ?>);
  major_meanGPArange.push(<?php echo($rows["GPA10"]); ?>);
    major_meanGPA.push({
    GPA10: parseFloat("<?php echo($rows["GPA10"]); ?>"),
    GPA25: parseFloat("<?php echo($rows["GPA25"]); ?>"),
    GPA50: parseFloat("<?php echo($rows["GPA50"]); ?>"),
    GPA75: parseFloat("<?php echo($rows["GPA75"]); ?>"),
    GPA90: parseFloat("<?php echo($rows["GPA90"]); ?>")
  });
 ...
 <!--  extract occupation character requirement -->
  ...
  salaryMedian, 
  salary75thPercentile,
  salary90thPercentile, description from  OccupationCha042016 group by occupationGroup,occupation";//OccupationCha042016
  $result = $link->query($sql);
  if ($result->num_rows <= 0) {
       echo "0 results";
     }
...
<!--  extract mean occupation character requirement -->
   ...
   while($rows = mysqli_fetch_assoc($result))
  {
    ?>
  <script>
  occupation_meansalaryrange.push(<?php echo($rows["salary90th"]); ?>);
  occupation_meansalaryrange.push(<?php echo($rows["salary10th"]); ?>);
  occupation_meansalary.push({
  salary10: parseInt("<?php echo($rows["salary10th"]); ?>"),
  salary25: parseInt("<?php echo($rows["salary25th"]); ?>"),
  salary50: parseInt("<?php echo($rows["salary50th"]); ?>"),
  salary75: parseInt("<?php echo($rows["salary75th"]); ?>"),
...
//old VQR bar ends
  ...
  function customYAxis(g) {
    ...
    g.selectAll(".tick:not(:only-of-type) line").attr("stroke", "#70ABD0").attr("stroke-width", 0.3).attr("stroke-dasharray", "2,2");
...
//left part start
 ...
 //major Salary
  ...
  var h = 100;
  var barnumber = 1;
  var Salarybarlength = major_meansalary.length;


  var leftSideCharts = svg.append("g").attr("id","leftSideCharts");
  var rightSideCharts = svg.append("g").attr("id","rightSideCharts");
  var majorSalarybar=leftSideCharts.selectAll(".majorSalarybar")
  .data(major_meansalary)
  .enter().append("g")
  .attr("class","majorSalarybar");
  majorSalarybar.append("rect")
      .attr("id",function (d,i) {
        var id=i+Salarybarlength*0;
        return "majorSalarybarid"+id;
      })
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+i * (w / Salarybarlength);
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.salary75 * 0.0015)+45;
      })
      .attr("width", function(d, i) { 
        return w / Salarybarlength - 1;
      })
      .attr("height", function(d,i) {
        return (d.salary75-d.salary25) * 0.0015;
      })
      .attr("fill", function(d,i) {
        return color2[i%Salarybarlength];
      }) 
      .append("title")
      .text(function(d,i) {
      if(i<Salarybarlength)return college_name.name[i]+":"+d.salary50;
      });

  majorSalarybar.append("rect")
      .attr("id",function (d,i) {
        var id=i+Salarybarlength*1;
        return "majorSalarybarid"+id;
      })
      .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / Salarybarlength)-0.5-1;//1/2 width of line is 1
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.salary90 * 0.0015)+45;
      })
      .attr("width", function(d, i) { 
        return 2;
      })
      .attr("height", function(d,i) {
        return (d.salary90-d.salary75)* 0.0015;
      })
      .attr("fill", function(d,i) {
        return color2[i%Salarybarlength];
      }) 
  ...
  majorSalarybar.append("rect")
      ...
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.salary50 * 0.0015)+45;
      })
      .attr("width", function(d, i) { 
        return (w / Salarybarlength) - 1;
      })
  ...
  majorSalarybar.append("rect")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / Salarybarlength)-0.5-1;
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.salary25 * 0.0015)+45;
      })
      .attr("width", function(d, i) { 
        return 2;
      })
      .attr("height", function(d,i) {
        return (d.salary25-d.salary10)* 0.0015;
      })
      .attr("fill", function(d,i) {
        return color2[i%Salarybarlength];
      }) 
      ...
      tmpy=chart_base_y-(accordion_bar_height+5)*1-d3.max(major_meansalaryrange)* 0.0015-d3.min(major_meansalaryrange)* 0.0015+45;// important***
      var y = d3.scaleLinear()
      .domain([d3.min(major_meansalaryrange), d3.max(major_meansalaryrange)])
      .range([d3.max(major_meansalaryrange)* 0.0015, d3.min(major_meansalaryrange)* 0.0015]);
      
      var yAxis = d3.axisRight(y)
      .tickSize(250)
      .ticks(3, ",f");
      axislocation.x.push(chart_base_xl);
      axislocation.y.push(tmpy);
      majorSalarybar.append("g")
      .attr("id","yaxis")
      .attr("visibility","hidden")
      .attr("transform", "translate(" + chart_base_xl + "," + tmpy+ ")")
      .call(customYAxis);



  var subSalarybarlength = major_salary.length;
 
  var submajorSalarybar=leftSideCharts.selectAll(".submajorSalarybar")
  .data(major_salary)
  .enter().append("g")
  ...
  submajorSalarybar.append("rect")
      .attr("id",function (d,i) {
        var id=i+subSalarybarlength*0;
        return "submajorSalarybarid"+id;
      })
      .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
         return 1;
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+i * (w / subSalarybarlength);
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.salary75 * 0.0015)+45;
      })
      .attr("width", function(d, i) { 
        return w / subSalarybarlength - 1;
      })
      .attr("height", function(d,i) {
        return 0;
      })
  ...
  submajorSalarybar.append("rect")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / subSalarybarlength)-0.5-1;//1/2 width of line is 1
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.salary90 * 0.0015)+45;
      })
  ...
  submajorSalarybar.append("rect")
     ...
     .attr("visibility","hidden")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / subSalarybarlength)-0.5-1;
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.salary25 * 0.0015)+45;
      })
  ...
  var h = 100;
  var barnumber = 1;
  var GPAbarlength = major_meanGPA.length;



  var majorGPAbar=leftSideCharts.selectAll(".majorGPAbar")
  .data(major_meanGPA)
  .enter().append("g")
  ...
  majorGPAbar.append("rect")
      .attr("id",function (d,i) {
        var id=i+GPAbarlength*0;
        return "majorGPAbarid"+id;
      })
      .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
         return 1;
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+i * (w / GPAbarlength);
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.GPA75*30)+100;
      })
      .attr("width", function(d, i) { 
        return w / GPAbarlength - 1;
      })
      .attr("height", function(d,i) {
        return (d.GPA75-d.GPA25) * 30;
      })
      .attr("fill", function(d,i) {
        return color2[i%GPAbarlength];
      }) 
      .append("title")
      .text(function(d,i) {
      if(i<GPAbarlength)return college_name.name[i]+":"+d.GPA50;
      });

  majorGPAbar.append("rect")
      .attr("id",function (d,i) {
        var id=i+GPAbarlength*1;
        return "majorGPAbarid"+id;
      })
     .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / GPAbarlength)-0.5-1;//1/2 width of line is 1
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.GPA90*30 )+100;
      })
      .attr("width", function(d, i) { 
        return 2;
      })
      .attr("height", function(d,i) {
        return (d.GPA90-d.GPA75)* 30;
      })
      .attr("fill", function(d,i) {
        return color2[i%GPAbarlength];
      }) 
  ...
  majorGPAbar.append("rect")
      ...
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.GPA50*30 )+100;
      })
      .attr("width", function(d, i) { 
        return (w / GPAbarlength) - 1;
      })
  ...
  majorGPAbar.append("rect")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / GPAbarlength)-0.5-1;
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.GPA25*30 )+100;
      })
      .attr("width", function(d, i) { 
        return 2;
      })
      .attr("height", function(d,i) {
        return (d.GPA25-d.GPA10)* 30;
      })
      .attr("fill", function(d,i) {
      ...
      tmpy=chart_base_y-(accordion_bar_height+5)*1-d3.max(major_meanGPArange)*30-d3.min(major_meanGPArange)*30+100;// important***
      var y = d3.scaleLinear()
      .domain([d3.min(major_meanGPArange), d3.max(major_meanGPArange)])
      .range([d3.max(major_meanGPArange)*30, d3.min(major_meanGPArange)*30]);
  ...
  var subGPAbarlength = major_GPA.length;
 
  var submajorGPAbar=leftSideCharts.selectAll(".submajorGPAbar")
  .data(major_GPA)
  .enter().append("g")
  .attr("class","submajorGPAbar");
  
  submajorGPAbar.append("rect")
      .attr("id",function (d,i) {
        var id=i+subGPAbarlength*0;
        return "submajorGPAbarid"+id;
      })
      .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
         return 1;
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+i * (w / subGPAbarlength);
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.GPA75*30 )+100;
      })
      .attr("width", function(d, i) { 
        return w / subGPAbarlength - 1;
      })
      .attr("height", function(d,i) {
        return 0;
      })
  ...
  submajorGPAbar.append("rect")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / subGPAbarlength)-0.5-1;//1/2 width of line is 1
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.GPA90*30 )+100;
      })
  ...
  submajorGPAbar.append("rect")
     ...
     .attr("visibility","hidden")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / subGPAbarlength)-0.5-1;
      })
      .attr("y", function(d,i) {
        return chart_base_y-(accordion_bar_height+5)*1- (d.GPA25*30)+100;
      })
      ...
      .attr("fill", function(d,i) {
        ...
        chart_img_id.push("");
        chart_y.push(0);
        chart_subclass_name.push(".submajorGPAbar");
        chart_sub_y.push(0);
        chart_text_y.push(0);
  ...
  var h = 100;
  var barnumber = 1;
  var SATbarlength = major_meanSAT.length;



  var majorSATbar=leftSideCharts.selectAll(".majorSATbar")
  .data(major_meanSAT)
  .enter().append("g")
  ...
  majorSATbar.append("rect")
      .attr("id",function (d,i) {
        var id=i+SATbarlength*0;
        return "majorSATbarid"+id;
      })
      .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
         return 1;
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+i * (w / SATbarlength);
      })
      .attr("y", function(d,i) {
        return chart_base_y+(accordion_bar_height+5)*1- (d.SAT75*0.1)+100;
      })
      .attr("width", function(d, i) { 
        return w / SATbarlength - 1;
      })
      .attr("height", function(d,i) {
        return (d.SAT75-d.SAT25) * 0.1;
      })
      .attr("fill", function(d,i) {
        return color2[i%SATbarlength];
      }) 
      .append("title")
      .text(function(d,i) {
      if(i<SATbarlength)return college_name.name[i]+":"+d.SAT50;
      });

  majorSATbar.append("rect")
      .attr("id",function (d,i) {
        var id=i+SATbarlength*1;
        return "majorSATbarid"+id;
      })
     .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / SATbarlength)-0.5-1;//1/2 width of line is 1
      })
      .attr("y", function(d,i) {
        return chart_base_y+(accordion_bar_height+5)*1- (d.SAT90*0.1 )+100;
      })
      .attr("width", function(d, i) { 
        return 2;
      })
      .attr("height", function(d,i) {
        return (d.SAT90-d.SAT75)* 0.1;
      })
      .attr("fill", function(d,i) {
        return color2[i%SATbarlength];
      }) 
  ...
  majorSATbar.append("rect")
      ...
      .attr("y", function(d,i) {
        return chart_base_y+(accordion_bar_height+5)*1- (d.SAT50*0.1 )+100;
      })
      .attr("width", function(d, i) { 
        return (w / SATbarlength) - 1;
      })
  ...
  majorSATbar.append("rect")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / SATbarlength)-0.5-1;
      })
      .attr("y", function(d,i) {
        return chart_base_y+(accordion_bar_height+5)*1- (d.SAT25*0.1 )+100;
      })
      .attr("width", function(d, i) { 
        return 2;
      })
      ...
      tmpy=chart_base_y+(accordion_bar_height+5)*1-d3.max(major_meanSATrange)*0.1-d3.min(major_meanSATrange)*0.1+100;// important***
      var y = d3.scaleLinear()
      .domain([d3.min(major_meanSATrange), d3.max(major_meanSATrange)])
      .range([d3.max(major_meanSATrange)*0.1, d3.min(major_meanSATrange)*0.1]);
      var yAxis = d3.axisRight(y)
      .tickSize(250)
      .ticks(3, ",f");
      axislocation.x.push(chart_base_xl);
      axislocation.y.push(tmpy);
      majorSATbar.append("g")
      .attr("id","yaxis")
      .attr("visibility","hidden")
      .attr("transform", "translate(" + chart_base_xl + "," + tmpy+ ")")
      .call(customYAxis);
  ...
  var subSATbarlength = major_SAT.length;
 
  var submajorSATbar=leftSideCharts.selectAll(".submajorSATbar")
  .data(major_SAT)
  .enter().append("g")
  .attr("class","submajorSATbar");
  
  submajorSATbar.append("rect")
      .attr("id",function (d,i) {
        var id=i+subSATbarlength*0;
        return "submajorSATbarid"+id;
      })
      .attr("visibility","hidden")
      .attr("opacity",function(d, i) {
         return 1;
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+i * (w / subSATbarlength);
      })
      .attr("y", function(d,i) {
        return chart_base_y+(accordion_bar_height+5)*1- (d.SAT75*0.1 )+100;
      })
      .attr("width", function(d, i) { 
        return w / subSATbarlength - 1;
      })
      .attr("height", function(d,i) {
        return 0;
      })
  ...
  submajorSATbar.append("rect")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / subSATbarlength)-0.5-1;//1/2 width of line is 1
      })
      .attr("y", function(d,i) {
        return chart_base_y+(accordion_bar_height+5)*1- (d.SAT90*0.1 )+100;
      })
  ...
  submajorSATbar.append("rect")
     ...
     .attr("visibility","hidden")
      ...
      .attr("x", function(d, i) {  
        return chart_base_xl+(i+0.5) * (w / subSATbarlength)-0.5-1;
      })
      .attr("y", function(d,i) {
        return chart_base_y+(accordion_bar_height+5)*1- (d.SAT25*0.1)+100;
      })
      ...
      .attr("fill", function(d,i) {
        ...
        chart_img_id.push("");
        chart_y.push(0);
        chart_subclass_name.push(".submajorSATbar");
        chart_sub_y.push(0);
        chart_text_y.push(0);
  ...
  for(var j = 3; j < 10 ; j++)
  {
    axislocation.x.push(0);
    axislocation.y.push(0);
    chart_name.push("");
...
//Occupation Salary
  ...
  var h = 100;
  var barnumber = 1;
  var Salarybarlength = occupation_meansalary.length;
  var occupationSalarybar=rightSideCharts.selectAll(".occupationSalarybar")
  .data(occupation_meansalary)
  .enter().append("g")
  .attr("class","occupationSalarybar");
      ...
      occupationSalarybar.append("rect")
          .attr("id",function (d,i) {
            var id=i+Salarybarlength*0;
                return "occupationSalarybarid"+id;
          })
          //.attr("opacity",0.0)
          .attr("visibility","hidden")
          .attr("opacity",function(d, i) {
          ...
          .attr("x", function(d, i) {  
            return chart_base_x+i * (w / Salarybarlength);
          })
          .attr("y", function(d,i) {
            return chart_base_y-(accordion_bar_height+5)*1- (d.salary75 * 0.0015)+65;
          })
          .attr("width", function(d, i) { 
            return w / Salarybarlength - 1;
          })
          ...
          .append("title")
          .text(function(d,i) {
            return occupationautomation.name[i]+":"+d.salary50;
          });
      occupationSalarybar.append("rect")
          .attr("id",function (d,i) {
            var id=i+Salarybarlength;
            return "occupationSalarybarid"+id;
          })
          //.attr("opacity",0.0)
          .attr("visibility","hidden")
          ...
          .attr("x", function(d, i) {  
            return chart_base_x+(i+0.5) * (w / Salarybarlength)-0.5-1;//1/2 width of line is 1
          })
          .attr("y", function(d,i) {
            return chart_base_y-(accordion_bar_height+5)*1- (d.salary90 * 0.0015)+65;
          })
          .attr("width", function(d, i) { 
            return 2;
          })
      ...
      occupationSalarybar.append("rect")
          ...
          .attr("y", function(d,i) {
            return chart_base_y-(accordion_bar_height+5)*1- (d.salary50 * 0.0015)+65;
          })
          .attr("width", function(d, i) { 
            return (w / Salarybarlength) - 1;
          })
      ...
      occupationSalarybar.append("rect")
          ...
          .attr("x", function(d, i) {  
            return chart_base_x+(i+0.5) * (w / Salarybarlength)-0.5-1;
          })
          .attr("y", function(d,i) {
            return chart_base_y-(accordion_bar_height+5)*1- (d.salary25 * 0.0015)+65;
          })
          .attr("width", function(d, i) { 
            return 2;
          })
          ...
          .append("title")
          .text(function(d,i) {
          if(i<13)return occupationautomation.name[i]+":"+d.salary50;
          });
          /*d3.selectAll(".occupationSalarybar")
          .attr("transform",function(d,i){
            return "scale(0.001,1)";
          })
          .attr("transform",function(d,i){
            return "translate(80,1)";
          })*/
          
          tmpy=chart_base_y-(accordion_bar_height+5)*1-d3.max(occupation_meansalaryrange)* 0.0015-d3.min(occupation_meansalaryrange)* 0.0015+65;// important***
          var y = d3.scaleLinear()
          .domain([d3.min(occupation_meansalaryrange), d3.max(occupation_meansalaryrange)])
          .range([d3.max(occupation_meansalaryrange)* 0.0015, d3.min(occupation_meansalaryrange)* 0.0015]);
          
          var yAxis = d3.axisRight(y)
          .tickSize(250)
          .ticks(3, ",f");

          axislocation.x.push(chart_base_x_axis);
          axislocation.y.push(tmpy);
          occupationSalarybar.append("g")
          .attr("id","yaxis")
          .attr("visibility","hidden")
          .attr("transform", "translate(" + chart_base_x_axis + "," + tmpy+ ")")
          .call(customYAxis);



  var subSalarybarlength = occupation_salary.length;

  var suboccupationSalarybar=rightSideCharts.selectAll(".suboccupationSalarybar")
  .data(occupation_salary)
  .enter().append("g")
  .attr("class","suboccupationSalarybar");

  d3.select(".suboccupationSalarybar").append("g");


      suboccupationSalarybar.append("rect")
          .attr("id",function (d,i) {
            var id=i+subSalarybarlength*0
            return "suboccupationSalarybarid"+id;
          })
          //.attr("opacity",0.0)
          .attr("visibility","hidden")
          .attr("opacity",function(d, i) {
          ...
          .attr("x", function(d, i) {  
            return chart_base_x+i * (w / subSalarybarlength);
          })
          .attr("y", function(d,i) {
            return chart_base_y-(accordion_bar_height+5)*1- (d.salary75 * 0.0015)+65;
          })
          ...
          .append("title")
          .text(function(d,i) {
            return automation.name[i]+":"+d.salary50;
          });
      suboccupationSalarybar.append("rect")
          .attr("id",function (d,i) {
            var id=i+subSalarybarlength*1
          ...
          .attr("x", function(d, i) {  
            return chart_base_x+(i+0.5) * (w / subSalarybarlength)-0.5-1;//1/2 width of line is 1
          })
          .attr("y", function(d,i) {
            return chart_base_y-(accordion_bar_height+5)*1- (d.salary90 * 0.0015)+65;  
          })
      ...
      suboccupationSalarybar.append("rect")
          ...
          .attr("x", function(d, i) {  
            return chart_base_x+(i+0.5) * (w / subSalarybarlength)-0.5-1;
          })
          .attr("y", function(d,i) {
            return chart_base_y-(accordion_bar_height+5)*1- (d.salary25 * 0.0015)+65;
          })
    ...
    for (var i = 0; i < occupationGroup.name.length; i++) {
      ...
      var OA = rightSideCharts.selectAll(".OA")
        ...
        .on("mouseover", function() {
         ...
         OA.append("text")
         .text("Probability Obsolete")
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+30+5)
         .attr("font-size", "15px")
         .attr("fill", "black");
         // zzcharttext_y.push(0);
        // axis for OA
        
        tmpy=chart_base_y-d3.max(occupationautomation.number)+20;
        var y = d3.scaleLinear()
        .domain([0, d3.max(occupationautomation.number)])
        .range([d3.max(occupationautomation.number), 0]);
...
//Create suboccupationautomation element
      ...
      var OSA = rightSideCharts.selectAll(".OSA")
        ...
        chart_name.push("Automation Sensitivity");
...
//OG
      var OG = rightSideCharts.selectAll(".OG")
        ...
        OG.append("text")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+1*(accordion_bar_height+5)+30+5)
         .attr("font-size", "15px")
         .attr("fill", "black");
         // zzcharttext_y.push(0);
         // axis for OG
        tmpy=chart_base_y+(accordion_bar_height+5)*1-20-d3.max(occupationglobalization.number)*0.5;
        var y = d3.scaleLinear()
        .domain([d3.min(occupationglobalization.number), d3.max(occupationglobalization.number)])
        .range([d3.max(occupationglobalization.number)*0.5-d3.min(occupationglobalization.number)*0.5, 0]);
...
//Create suboccupationglobalization element
      ...
      var OSG = rightSideCharts.selectAll(".OSG")
         ...
         .attr("y", function(d) {
            ...
            return chart_base_y+(accordion_bar_height+5)*1-(accordion_bar_height+5)+20 ;
            })
         .attr("height", function(d) {
         ...
         // zzsubchart.push(".OSG");
         // zzsubchart_y.push(0);
        chart_name.push("Globalization Sensitivity");
...
//OM
      var OM = rightSideCharts.selectAll(".OM")
        ...
        OM.append("text")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+20+2*(accordion_bar_height+5)+30)
         .attr("font-family", "sans-serif")
         .attr("font-size", "15px")
         .attr("fill", "gray");
         // zzcharttext_y.push(0);
        tmpy=chart_base_y+20+(accordion_bar_height+5)*2-d3.max(occupationmarried.number)* 0.75;
        var y = d3.scaleLinear()
        .domain([0, d3.max(occupationmarried.number)])
        .range([d3.max(occupationmarried.number)* 0.75, 0]);
...
//OSM
      ...
      var OSM = rightSideCharts.selectAll(".OSM")
         .data(married.number)
         .enter().append("g")
         .attr("class","OSM");
         ...
         .attr("y", function(d) {
          if(d<0) return chart_base_y+20+(accordion_bar_height+5)*2;
            return chart_base_y+20+(accordion_bar_height+5)*2 - (d * 0);
            })
         .attr("height", function(d) {
            if(d<0) return d * (-0);
            return d * 0;
...
//Oblack
      var Oblack = rightSideCharts.selectAll(".Oblack")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+20+3*(accordion_bar_height+5)+30)
         .attr("font-family", "sans-serif")
         .attr("font-size", "15px")
         .attr("fill", "gray");
         // zzcharttext_y.push(0);
         console.log(occupationblackPercent.number);
        tmpy=chart_base_y+20+(accordion_bar_height+5)*3-d3.max(occupationblackPercent.number)* 3-d3.min(occupationblackPercent.number)* 3;
        var y = d3.scaleLinear()
        .domain([d3.min(occupationblackPercent.number), d3.max(occupationblackPercent.number)])
        .range([d3.max(occupationblackPercent.number)* 3, d3.min(occupationblackPercent.number)* 3]);
        ...
        .call(customYAxis);
//OSblack
      var OSblack = rightSideCharts.selectAll(".OSblack")
         ...
         .attr("y", function(d) {
          if(d<0) return chart_base_y+20+(accordion_bar_height+5)*3;
            return chart_base_y+20+(accordion_bar_height+5)*3 - (d * 0);
         })
...
//Oasian
      var Oasian = rightSideCharts.selectAll(".Oasian")
        ...
        Oasian.append("text")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+20+4*(accordion_bar_height+5)+30)
         .attr("font-family", "sans-serif")
         .attr("font-size", "15px")
         .attr("fill", "gray");
         // zzcharttext_y.push(0);
        tmpy=chart_base_y+20+(accordion_bar_height+5)*4-d3.max(occupationasianPercent.number)* 3;
        var y = d3.scaleLinear()
        .domain([0, d3.max(occupationasianPercent.number)])
        .range([d3.max(occupationasianPercent.number)* 3, 0]);
        ...
        .call(customYAxis);
//OSasian
      var OSasian = rightSideCharts.selectAll(".OSasian")
         ...
         .attr("y", function(d) {
          if(d<0) return 730;
            return chart_base_y+20+(accordion_bar_height+5)*4 - (d * 0);
         })
...
//Ohispanic
      var Ohispanic = rightSideCharts.selectAll(".Ohispanic")
        ...
        Ohispanic.append("text")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+20+5*(accordion_bar_height+5)+30)
         .attr("font-family", "sans-serif")
         .attr("font-size", "15px")
         .attr("fill", "gray");
         // zzcharttext_y.push(0);
        tmpy=chart_base_y+20+(accordion_bar_height+5)*5-d3.max(occupationhispanicPercent.number)* 3;
        var y = d3.scaleLinear()
        .domain([0, d3.max(occupationhispanicPercent.number)* 3])
        .range([d3.max(occupationhispanicPercent.number)* 3, 0]);
        ...
        .call(customYAxis);
//OShispanic
      var OShispanic = rightSideCharts.selectAll(".OShispanic")
         ...
         .attr("y", function(d) {
          if(d<0) return 730;
            return chart_base_y+20+(accordion_bar_height+5)*5 - (d * 0);
         })
...
//Ofemale
      var Ofemale = rightSideCharts.selectAll(".Ofemale")
        ...
        Ofemale.append("text")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+20+6*(accordion_bar_height+5)+30)
         .attr("font-family", "sans-serif")
         .attr("font-size", "15px")
         .attr("fill", "gray");
         // zzcharttext_y.push(0);
        tmpy=chart_base_y+20+(accordion_bar_height+5)*6-d3.max(occupationfemalePercent.number)* 0.75;
        var y = d3.scaleLinear()
        .domain([0, d3.max(occupationfemalePercent.number)* 0.75])
        .range([d3.max(occupationfemalePercent.number)* 0.75, 0]);
        ...
        .call(customYAxis);
//OSfemale
      var OSfemale = rightSideCharts.selectAll(".OSfemale")
         ...
         .attr("y", function(d) {
          if(d<0) return 730;
            return chart_base_y+20+(accordion_bar_height+5)*6 - (d * 0);
         })
...
//OforeignBorn
      var OforeignBorn = rightSideCharts.selectAll(".OforeignBorn")
        ...
        OforeignBorn.append("text")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+20+7*(accordion_bar_height+5)+30)
         .attr("font-family", "sans-serif")
         .attr("font-size", "15px")
         .attr("fill", "gray");
         // zzcharttext_y.push(0);
        tmpy=chart_base_y+20+(accordion_bar_height+5)*7-d3.max(occupationforeignBornPercent.number)* 3;
        var y = d3.scaleLinear()
        .domain([0, d3.max(occupationforeignBornPercent.number)* 3])
        .range([d3.max(occupationforeignBornPercent.number)* 3, 0]);
        ...
        axislocation.x.push(chart_base_x_axis);
        axislocation.y.push(tmpy);
        OforeignBorn.append("g")
        .attr("id","yaxis")
        .attr("visibility","hidden")
        .attr("transform", "translate(" + chart_base_x_axis + "," + tmpy + ")")
        .call(customYAxis);
//OSforeignBorn
      var OSforeignBorn = rightSideCharts.selectAll(".OSforeignBorn")
         ...
         .attr("y", function(d) {
          if(d<0) return 730;
            return chart_base_y+20+(accordion_bar_height+5)*7 - (d * 0);
         })
...
//OaverageHoursOfWorkOrWeek
      var OaverageHoursOfWorkOrWeek = rightSideCharts.selectAll(".OaverageHoursOfWorkOrWeek")
        ...
        OaverageHoursOfWorkOrWeek.append("text")
         ...
         .attr("visibility","hidden")
         .attr("x",  chart_base_x)
         .attr("y", chart_base_y+20+8*(accordion_bar_height+5)+30)
         .attr("font-family", "sans-serif")
         .attr("font-size", "15px")
         .attr("fill", "gray");
         // zzcharttext_y.push(0)//OSaverageHoursOfWorkOrWeek
        tmpy=chart_base_y+20+(accordion_bar_height+5)*8-d3.max(occupationaverageHoursOfWorkOrWeek.number)* 1.5;
        var y = d3.scaleLinear()
        .domain([0, d3.max(occupationaverageHoursOfWorkOrWeek.number)* 1.5])
        .range([d3.max(occupationaverageHoursOfWorkOrWeek.number)* 1.5, 0]);
        ...
        axislocation.x.push(chart_base_x_axis);
        axislocation.y.push(tmpy);
        OaverageHoursOfWorkOrWeek.append("g")
        .attr("id","yaxis")
        .attr("visibility","hidden")
        .attr("transform", "translate(" + chart_base_x_axis + "," + tmpy + ")")
        .call(customYAxis);

      var OSaverageHoursOfWorkOrWeek = rightSideCharts.selectAll(".OSaverageHoursOfWorkOrWeek")
         ...
         .attr("y", function(d) {
          if(d<0) return 730;
            return chart_base_y+20+(accordion_bar_height+5)*8 - (d * 0);
         })
        ...
        chart_name.push("Average Hours of Work/week");
  ...
  var centralChart = svg.append("g").attr("id","centralChart");
        
    var collegeMainbar=centralChart.selectAll(".collegeMainbar")
        .data(college.name)
        .enter().append("g")
 ...
 var collegeName = collegeMainbar.append("g")
  ...
  initX(occupationGroup, occupationGroupX, occupationGroupWidth, ogscale);
    var occupationMainbar=centralChart.selectAll(".occupationMainbar")
 ...
 var occupationName = occupationMainbar.append("g")
  ...
  var collapseButton=centralChart.selectAll(".collapseButton")
    .data(collapseButtonData)
        .enter().append("g");


    var collegeCollapseButton=centralChart.selectAll(".collegeCollapseButton")
      .data(collapseButtonData)
      .enter().append("g");
  ...
  var occupationCollapseButton=centralChart.selectAll(".occupationCollapseButton")
      .data(collapseButtonData)
      .enter().append("g");
